...
```shell
curl --request POST \
  --url <URL>/api/v1/dashboard/create_borrower/ \
...
`POST <URL>/api/v1/dashboard/create_borrower`
...
```shell
curl --request GET \
  --url <URL>/api/v1/parser/supported_banks \
  --header 'Content-Type: application/json' \
  --header 'access-id: your_access_id' \
  --header 'access-key: your_access_key'
```
...
`GET <URL>/api/v1/parser/supported_banks`
...
<aside class="notice"> 
  The response for this api is the same as v1, with the only difference being the inclusion 
  of an `errorTransactions` object.
</aside>
...
`POST <URL>/api/v1/parser/reanalyse/`
